...
export default function Sidebar() {
    const { setIsExpenseModalOpen, setExpenseDate, handleLogout, isMobileSidebarOpen, setIsMobileSidebarOpen, availableFunds, totalExpenses } = useDashboard()
        ...
        <>
            ...
            {isMobileSidebarOpen && (
                <div
                    ...
                    className="fixed inset-0 z-40 bg-[#0a1120]/60 backdrop-blur-xs md:hidden cursor-pointer"
                />
            )}

            <aside className={`fixed inset-y-0 left-0 z-50 w-64 bg-[#131b2e] border-r border-slate-800/40 p-6 flex flex-col transition-transform duration-300 transform ${
                isMobileSidebarOpen ? "translate-x-0" : "-translate-x-full"
            } md:translate-x-0 md:relative md:flex md:h-screen md:sticky md:top-0 md:flex-shrink-0`}>
                <div className="flex items-center justify-between gap-3 mb-8">
                    ...
                    <button
                        onClick={() => setIsMobileSidebarOpen(false)}
                        className="md:hidden p-1.5 text-slate-400 hover:text-white hover:bg-[#252f48] rounded-xl cursor-pointer"
                    >
                ...
                <nav className="flex-1 space-y-2">
                    <Link
                        href="/"
                        onClick={() => setIsMobileSidebarOpen(false)}
                        className={`flex items-center gap-3 px-4 py-3 rounded-xl font-medium transition-colors cursor-pointer ${pathname === "/"
                    ...
                    <Link
                        href="/profile"
                        onClick={() => setIsMobileSidebarOpen(false)}
                        className={`flex items-center gap-3 px-4 py-3 rounded-xl font-medium transition-colors cursor-pointer ${pathname === "/profile"
                                ? "bg-[#252f48] text-white"
                                : "text-slate-400 hover:text-white hover:bg-[#1a233a]/50"
                            }`}
                    >
                ...
                <div className="space-y-3 mb-6">
                    <div className="bg-[#1b253b]/40 border border-slate-800/60 px-4 py-3 rounded-2xl">
                        <span className="block text-[9px] font-bold text-slate-400 uppercase tracking-wider mb-0.5">Dostępne środki</span>
                        <span className={`text-base font-extrabold ${availableFunds >= 0 ? "text-white" : "text-red-400"}`}>
                            {availableFunds.toFixed(2)} PLN
                        </span>
                    </div>
                    <div className="bg-[#1b253b]/40 border border-slate-800/60 px-4 py-3 rounded-2xl">
                        <span className="block text-[9px] font-bold text-slate-400 uppercase tracking-wider mb-0.5">Wydatki</span>
